...
def enycrypt_message(public_key, message):
    n, e = public_key
    message_bytes = message.encode()
    message_int = int.from_bytes(message_bytes, byteorder='big')
...
def decrypt_message(private_key, encrypted_message):
    ...
    decrypted_bytes = int.to_bytes(decrypted_message, length=(decrypted_message.bit_length() + 7) // 8, byteorder='big')
    decrypted = decrypted_bytes.decode()
    return decrypted

print(1)
# Example usage
key_size = 1024  # Adjust the key size as needed
private_key, public_key = generate_keypair(key_size)
print(2)
message = "Your file content here"
signature = sign_message(private_key, message)
print(3)
enycrypt_text = enycrypt_message(public_key, message)

print("Encrypted message:", enycrypt_text)
decrypted_text = decrypt_message(private_key, enycrypt_text)
